...
# Convert 'Date' to timestamp (numeric value)
data['Date_numeric'] = pd.to_datetime(data['Date']).astype(int) / 10**9  # Convert to seconds

# Extract Date Components (optional, based on your preference)
...
X = data.drop(columns=['Traffic_Volume', 'Date'], errors='ignore')  # Drop original 'Date' column
...
if uploaded_file:
    ...
    # Convert 'Date' to timestamp for a numeric value
    input_data['Date_numeric'] = pd.to_datetime(input_data['Date']).astype(int) / 10**9  # Unix timestamp
