...
data['Hour'] = data['Date'].dt.hour if 'Hour' in data.columns else 0  # Add Hour if available
...
# StatsModels: Adding Time-Related Features to the Model
X_time_features = data[['Year', 'Month', 'Day', 'Weekday', 'Hour']]  # Use time-related features
X_time_features = sm.add_constant(X_time_features)  # Add constant for statsmodels

# Fit the regression model using statsmodels
statsmodel = sm.OLS(y, X_time_features).fit()

# Save the statsmodels regression model
statsmodel_path = r"C:\\Advanced projects\\Bangalore_Traffic\\traffic_statsmodel.pkl"
with open(statsmodel_path, 'wb') as f:
    pickle.dump(statsmodel, f)
...
if uploaded_file is not None:
    ...
    try:
        ...
        input_data['Hour'] = input_data['Date'].dt.hour if 'Hour' in input_data.columns else 0
        ...
        # StatsModel: Add time-related features and make predictions
        input_data_time_features = input_data[['Year', 'Month', 'Day', 'Weekday', 'Hour']]
        input_data_time_features = sm.add_constant(input_data_time_features)

        # Load the saved statsmodels model
        with open(statsmodel_path, 'rb') as f:
            statsmodel_loaded = pickle.load(f)

        # Make predictions using the statsmodels model
        predictions_statsmodel = statsmodel_loaded.predict(input_data_time_features)
        input_data['Traffic_Volume'] = predictions_statsmodel

        # User selects a simple model or TPOT model
        model_choice = st.selectbox("Select a regression model", list(simple_models.keys()) + ["Best Pipeline (TPOT)", "StatsModel"])
        ...
        elif model_choice == "StatsModel":
            # Use the statsmodels regression model
            selected_model = statsmodel_loaded
        else:
