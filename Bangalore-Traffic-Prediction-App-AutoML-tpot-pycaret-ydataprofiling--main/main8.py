...
def train_sarima_model(data):
    # Use DateTime-related features (Year, Month, Day, Weekday) as exogenous variables
    exog = data[['Year', 'Month', 'Day', 'Weekday']]
    target = data['Traffic_Volume']
    
    # Fit SARIMAX model
    sarima_model = SARIMAX(target, exog=exog, order=(1, 1, 1), seasonal_order=(1, 1, 1, 12))
    sarima_results = sarima_model.fit(disp=False)
    return sarima_results

# Train the SARIMA model
sarima_results = train_sarima_model(data)
