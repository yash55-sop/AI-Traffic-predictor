...
for i, (pipeline_name, pipeline_details) in enumerate(tpot.evaluated_individuals_.items(), 1):
    ...
    pipeline_paths.append((pipeline_name, pipeline_file))
...
if uploaded_file is not None:
    ...
    try:
        ...
        pipeline_options = {name: path for name, path in pipeline_paths}
        selected_pipeline_name = st.selectbox("Select a pipeline for prediction:", list(pipeline_options.keys()))
        selected_pipeline_path = pipeline_options[selected_pipeline_name]

        with open(selected_pipeline_path, 'rb') as f:
